...
- (id)initWithFrame:(CGRect)frame
{
    ...
    @try {
        if (self = [super initWithFrame:frame])
        {
            ...
            UILabel *lblFalse;
            lblFalse=[[UILabel alloc]initWithFrame:CGRectMake((cgSize.width/2 - 80),(cgSize.height/2 - 60),cgSize.width/2,60)];
...
- (id)initWithFrame:(CGRect)frame label: (NSString *)theLabel
{
    ...
    @try {
        if (self = [super initWithFrame:frame])
        {
            ...
            [self addSubview:loadingView];
            UILabel *lblFalse;
            lblFalse=[[UILabel alloc]initWithFrame:CGRectMake(2,(cgSize.height/2 - 60),cgSize.width-4,60)];
            lblFalse.font=[UIFont fontWithName:@"Verdana" size:20];
            lblFalse.textColor=[UIColor whiteColor];
            lblFalse.backgroundColor = [UIColor colorWithRed:0.957 green:0.486 blue:0.173 alpha:1.000];
            lblFalse.textAlignment=UITextAlignmentRight;
            ...
            lblLoadingLabel = [[UILabel alloc] initWithFrame:CGRectMake(50, 20, 230, indicatorViewHeight)];
            lblLoadingLabel.text = theLabel;		//Loading label for other classess
