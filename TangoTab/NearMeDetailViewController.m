...
@implementation NearMeDetailViewController
...
@synthesize backBarButton,postTwitterViewCtrl,postFacebookViewCtrl;
...
- (void)viewDidLoad
{
    ...
    [dateAndTimeLabel setText:[NSString stringWithFormat:@"%@, %@ to %@",resultDate,[sharedDelegate.nearMeDetailDict valueForKey:@"available_start_time"],[sharedDelegate.nearMeDetailDict valueForKey:@"available_end_time"]]];
     self.navigationBar.topItem.title = [sharedDelegate currentServer:@"Offer"];
...
-(IBAction)fixDealButtonClicked:(id)sender{
    ...
    NSDictionary *dictionary = [NSDictionary dictionaryWithObjectsAndKeys:
     ...
     @"Claim Now Button",
     nil];
    [FlurryAnalytics logEvent:@"Claim Now" withParameters:dictionary];
    ...
    @try {  
        ...
        SilverPop *webService = [[SilverPop alloc] init];
        BOOL result = [webService invokeWithDealId: [sharedDelegate.nearMeDetailDict valueForKey:@"id"]];
        

         NSString *serviceURL = [NSString stringWithFormat:@"%@/deals/insertdeal?emailId=%@&timestamp=%@&dealId=%@&orderedtimestamp=",sharedDelegate.activeServerUrl,[sharedDelegate.userdetails valueForKey:@"login"],dateandtime,[sharedDelegate.nearMeDetailDict valueForKey:@"id"]];
...
-(void)parserDidEndDocument:(NSXMLParser *)parser{
  ...
  //      UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"TangoTab" message:dealStatus delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
    //    [alert show];
    //    [alert setTag:22];
    //    [alert release];
    }
...
{    
    if (sharedDelegate.isNotReachable == YES) {
        ...
        @try {
            ...
            mapViewCtrl.placesMapArray = array;
            [array release];
...
//    
//    if (buttonIndex == 0) {
//        FaceBook *obj = [FaceBook sharedDataSource];
//        if ( ((obj.fbGraph.accessToken == nil) || ([obj.fbGraph.accessToken length] == 0))) {            
//            [obj fbConntect:self.view];
//            [obj setDelegate:self];
//        }
//        else {        
//            [self postfacebookviewcontroller];
//            
//       }
//    }
//    else if (buttonIndex == 1) {
//        Twitter *obj1 = [Twitter sharedDataSource];
//        if ([obj1.engine.username length] == 0 &&[[sharedDelegate.searchingradies valueForKey:@"twitterLogin"] length] ==0) {
//            UIViewController *vie =  [obj1 twitterLogin];
//            [obj1 setDelegate1:self];
//            
//            [self presentModalViewController:vie animated:YES];
//            
//        }
//        else {
//            
//            [self tweetwitterviewcontrolle];
//        }
//    }
...
//{
//    FaceBook *obj = [FaceBook sharedDataSource];
//   postFacebookViewCtrl = [[PostFacebookViewController alloc]initWithNibName:@"PostFacebookViewController" bundle:[NSBundle mainBundle]];
//    postFacebookViewCtrl.fbGraph = obj.fbGraph;
//    [self.navigationController pushViewController:postFacebookViewCtrl animated:YES];
//}
//
//-(void)tweetwitterviewcontrolle
//{
//    Twitter *obj1 = [Twitter sharedDataSource];
//    postTwitterViewCtrl = [[PostTwitterViewController alloc] initWithNibName:@"PostTwitterViewController" bundle:[NSBundle mainBundle]];
//    postTwitterViewCtrl.engine = obj1.engine;
//    [self.navigationController pushViewController:postTwitterViewCtrl animated:YES];
//    
//}
