...
- (void)urlAction:(NSNotification *)notif
{
    ...
    debug_NSLog(@"Received Notification - URL Action");
    if ([sharedDelegate.urlActionController isEqualToString: @"searchViewController"])
    {
        UITabBarController *tab = [self tabBarController];
        [tab setSelectedIndex: 1]; // incase search tab is already visible
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                            selector:@selector(urlAction:)
                                            name:NotifyUrlAction object:nil];
    return self;
}
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    self.navigationBar.topItem.title = [sharedDelegate currentServer:@"Near Me"];
...
- (void)refresh:(NSDictionary *)addressDict {
    ...
    NSString *verString = [NSString stringWithFormat:@"%@",sharedDelegate.versionString];
    serverURL = [[NSString alloc]initWithFormat:@"%@/deals/search?type=A&city=%@&zipcode=%@&coordinate=%@&searchingradius=%@&pageIndex=0&noOfdeals=0&restName=&address=&version=%@&userId=%@",sharedDelegate.activeServerUrl,[arr objectAtIndex:0],[arr objectAtIndex:1],[arr objectAtIndex:2],[sharedDelegate.searchingradies objectForKey:@"radies"],verString,[sharedDelegate.userdetails  valueForKey:@"userId"]];
