...
@synthesize deal_name,isconsumershownup,host_manager_emailid,deal_description,deal_restrictions,start_time,end_time,business_name,reserved_time_stamp,address,image_url,noOfdeals,Details,serverURL,deal_id,deal_manager_emailid,con_res_id,longitude,latitude,current_date;
...
@synthesize checkView,mapviewcontroller;
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if (appdelegate.myOffersUpdate==NO) {
        ...
        serverURL = [[NSString alloc]initWithFormat:@"%@/mydeals/alldeals?emailId=%@&password=%@&noOfdeals=0&pageIndex=0",appdelegate.activeServerUrl,[appdelegate.userdetails objectForKey:@"login"],[appdelegate.userdetails objectForKey:@"psw"]];
    ...
    self.navigationBar.topItem.title = [appdelegate currentServer:@"My Offers"];
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict{
	...
	if ([elementName isEqualToString:@"detail"]) {
        ...
        current_date =[[NSMutableString alloc]init];
    }
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName{ 
    if ([elementName isEqualToString:@"detail"]) {
        ...
        [Details setValue:latitude forKey:@"latitude"];
        [Details setValue:longitude forKey:@"longitude"];
        [Details setValue:current_date forKey:@"current_date"];
...
- (void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string{
    ...
    if ([currentElement isEqualToString:@"con_res_id"]) {
        ...
        [latitude appendString:terstring];
    }
    else if ([currentElement isEqualToString:@"longitude"]) {
        [longitude appendString:terstring];
    }
    else if ([currentElement isEqualToString:@"current_date"]) {
        [current_date appendString:terstring];
    }
}
...
-(void)parserDidEndDocument:(NSXMLParser *)parser{
    ...
    NSMutableArray *dataforlocal = [[NSMutableArray alloc] init];
    [[UIApplication sharedApplication] cancelAllLocalNotifications];
    [appdelegate.searchingradies setValue:@"" forKey:@"dataforlocal"];
    [appdelegate.searchingradies synchronize];
    for (int i=0; i < [myOffersArray count] ; i++) {
        
        NSDictionary *tem = [myOffersArray objectAtIndex:i];
        
        NSString *resvdateStrig = [NSString stringWithFormat:@"%@",[tem valueForKey:@"reserved_time_stamp"]];   
        
        NSArray *resvdateArray = [resvdateStrig componentsSeparatedByString:@" "];
        
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
       
        [dateFormatter setDateFormat:@"yyyy-MM-dd hh:mm a"];
        
        NSString *dealDateStrig = [NSString stringWithFormat:@"%@ %@",[resvdateArray objectAtIndex:0],[tem valueForKey:@"end_time"]];
       
         // debug dealDateStrig = @"2012-10-22 07:00 PM";
        NSDate *dealDate = [dateFormatter dateFromString:dealDateStrig];
        ...
        NSString *dateStrig = [NSString stringWithFormat:@"%@",[resvdateArray objectAtIndex:0]];
        
        NSDate *date = [dateFormatter2 dateFromString:dateStrig];
        
        NSDateFormatter *dateformstring1 = [[NSDateFormatter alloc] init];
        [dateformstring1 setDateFormat:@"MM-dd-yyyy"];
        
        NSString *resultDateString = [dateformstring1 stringFromDate:date];
        
        NSString *userName = [NSString stringWithFormat:@"%@ %@",[appdelegate.searchingradies objectForKey:@"firstname"],[appdelegate.searchingradies objectForKey:@"last_name"]];
        
        NSDateFormatter *dateFormatter3 = [[NSDateFormatter alloc] init];
        [dateFormatter3 setDateFormat:@"yyyy/MM/dd hh:mm:ss a"];
        NSDate *currDate = [dateFormatter3 dateFromString:appdelegate.currentDateString];
        ...
        // debug  resultDate = @"2012-10-22 09:00:00 PM";
        NSDate *currDate1 = [dateformstring dateFromString:resultDate];
        
        
        NSString *InstallDate = [appdelegate.searchingradies objectForKey:@"installDate"];
        int checkStats= [[tem objectForKey:@"isconsumershownup"]integerValue];
        
        if ([dealDate compare:[appdelegate.searchingradies objectForKey:@"installDate"]] == NSOrderedDescending) {
                        
            if (!checkStats && [dealDate compare:[currDate1 dateByAddingTimeInterval:-14 * (24*60*60)]] == NSOrderedDescending) {
                
                NSString *convString = [NSString stringWithFormat:@"We hope you enjoyed your visit to %@ on %@.  Please let us know if you were able to attend.",[tem objectForKey:@"business_name"],resultDateString];
                //Time
                UILocalNotification *localNotification = [[UILocalNotification alloc] init];
                if (!localNotification) 
                    return;
                localNotification.fireDate = [dealDate dateByAddingTimeInterval:60*60];
                localNotification.alertAction = @"Confirm";
                localNotification.alertBody   = convString;
                localNotification.timeZone = [NSTimeZone defaultTimeZone];
                localNotification.soundName   = UILocalNotificationDefaultSoundName;
                [localNotification setHasAction:YES];
                NSDictionary *infoDict = [NSDictionary dictionaryWithObjectsAndKeys:convString,@"message",tem,@"dic", userName,@"userName",resultDateString,@"dealDate", [tem objectForKey:@"business_name"],@"busname",[tem objectForKey:@"isconsumershownup"],@"isconsu",nil];
                localNotification.userInfo = infoDict;
                [[UIApplication sharedApplication] scheduleLocalNotification:localNotification];
                NSData *data = [NSKeyedArchiver archivedDataWithRootObject:localNotification];
                [dataforlocal addObject:data];
                [localNotification release]; 
                 debug_NSLog(@"local notification created -> %@",localNotification);
            } 
        }
    }
    [appdelegate.searchingradies setValue:dataforlocal forKey:@"dataforlocal"];
    [appdelegate.searchingradies synchronize];
    [dataforlocal release];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (searching && [searchingArray count] > [indexPath row]){
        ...
        checkinStatus = [[[myOffersArray objectAtIndex:indexPath.row ]objectForKey:@"isconsumershownup"]integerValue];
//        NSString *conformationId;
//        NSString *currdateStrig = [NSString stringWithFormat:@"%@",[[myOffersArray objectAtIndex:indexPath.row ] valueForKey:@"current_date"]];    
//        NSArray *currdatArray = [currdateStrig componentsSeparatedByString:@" "];
//        
//        NSString *revdateStrig = [NSString stringWithFormat:@"%@",[[myOffersArray objectAtIndex:indexPath.row ]valueForKey:@"reserved_time_stamp"]];    
//        NSArray *resdatArray2 = [revdateStrig componentsSeparatedByString:@" "];
//        if ([[currdatArray objectAtIndex:0] isEqualToString:[resdatArray2 objectAtIndex:0]]) {
//            if (checkinStatus) {
//                conformationId=[NSString stringWithFormat:@"Used/Expired: %@",[[myOffersArray objectAtIndex:indexPath.row] objectForKey:@"con_res_id"]];
//                [customtableviewcell.backgroundImage setImage:[UIImage imageNamed:@"gray.png"]];
//            }
//            else {
//                conformationId=[NSString stringWithFormat:@"Confirmation Code: %@",[[myOffersArray objectAtIndex:indexPath.row] objectForKey:@"con_res_id"]];
//            }
//        }
//        else {
//            conformationId=[NSString stringWithFormat:@"Used/Expired: %@",[[myOffersArray objectAtIndex:indexPath.row] objectForKey:@"con_res_id"]];
...
-(void)showMoreAction:(id)sender{
    ...
    if ([myOffersArray count]!=noOfOffers) {
        ...
        serverURL = [[NSString alloc]initWithFormat:@"%@/mydeals/alldeals?emailId=%@&password=%@&noOfdeals=0&pageIndex=%i",appdelegate.activeServerUrl,[appdelegate.userdetails objectForKey:@"login"],[appdelegate.userdetails objectForKey:@"psw"],  pageIndex];
