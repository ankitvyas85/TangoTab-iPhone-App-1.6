...
+ (void)initialize
{
	if (self == [ASIHTTPRequest class]) {
		...
		ASIRequestTimedOutError = [[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Cannot connect to TangoTab service. This might be due to your data connection. If this problem persists, please notify TangoTab at help@tangotab.com.",NSLocalizedDescriptionKey,nil]] retain];	
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	NSString *connTimeOut = [[NSUserDefaults standardUserDefaults] stringForKey:@"textConnTime_key"];
	if(appDelegate.isConnTimeOut)
	{
		[self setTimeOutSeconds:60];
		appDelegate.isConnTimeOut = NO;
	}
	if(connTimeOut!=nil || [connTimeOut isEqualToString:@""])
		[self setTimeOutSeconds:[connTimeOut intValue]];
	else 
...
//	}
//	if (bandwidthThrottlingLock) {
//		[bandwidthThrottlingLock release];
//	}
//	if (sessionCookiesLock) {
//		[sessionCookiesLock release];
//	}
//	if (delegateAuthenticationLock) {
//		[delegateAuthenticationLock release];
//	}
...
{
	NSURLCredential *authenticationCredentials = [NSURLCredential credentialWithUser:[newCredentials objectForKey:(NSString *)kCFHTTPAuthenticationUsername] password:[newCredentials objectForKey:(NSString *)kCFHTTPAuthenticationPassword]
...
- (void)handleStreamError
{
	...
	if (![self error]) { // We may already have handled this error
		...
		NSString *reason = @"It appears there is no data connection, please check your settings.";
		...
		if ([[underlyingError domain] isEqualToString:NSOSStatusErrorDomain]) {
			if ([underlyingError code] <= -9800 && [underlyingError code] >= -9818) {
				reason = [NSString stringWithFormat:@"%@: SSL problem (possibily a bad/expired/self-signed certificate), \n%@",reason, underlyingError];
			}
		}
	//	NSException *anException = [NSException exceptionWithName:@"HTTP Stream Error" reason:reason userInfo:[NSDictionary dictionaryWithObjectsAndKeys:reason,NSLocalizedDescriptionKey,underlyingError,NSUnderlyingErrorKey,nil]];
		//[ErrorStack sendException:anException];
	...
	//	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Network error" message:reason delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
//		[alert show];
//		[alert release];
	}
    [super cancel];
