//
//  MYDealsReviewsViewController.m
//  TangoTab
//
...
- (void)viewDidLoad {
	...
	//UIBarButtonItem *addReview = [[UIBarButtonItem alloc] initWithTitle:@"Add Review" style:UIBarButtonItemStyleBordered target:self action:@selector(addReviewButtonClicked:)];
...
//	[addReview release];
	...
	[nc addObserver:self selector:@selector(dataParserError:) name:@"myDealsReviews_data_Parser_Error" object:nil];

	
	elementStack = [[NSMutableArray alloc] initWithObjects:@"reviews",@"review",@"user_rating",@"user_comment",@"errorMessage",nil];
	myDealsReviewsXmlParser = [[MyDealsReviewsXmlParser alloc] initWithElementStack:elementStack];	
...
//	TangoTabAppDelegate *appDelegate = (TangoTabAppDelegate *)[[UIApplication sharedApplication] delegate];
	...
	serviceURL = [NSString stringWithFormat:@"%@/deals/reviews?dealId=%@",SERVER_URL,[dealDic valueForKey:@"deal_id"]];
	[myDealsReviewsXmlParser loadFromURL:serviceURL];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MYDealsReviewsTableViewControllerCell *cell =(MYDealsReviewsTableViewControllerCell *) [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell == nil) {
		[[NSBundle mainBundle] loadNibNamed:@"MYDealsReviewsTableViewControllerCell" owner:self options:nil];
...
- (void)dealloc {
	[dealDic release];
	[noDataLabel release];
	//[myDealsReviewsXmlParser release];
	[activityIndicatorView release];
	...
	[[NSNotificationCenter defaultCenter] removeObserver:self name:@"myDealsReviews_data_Parser_Error" object:nil];
