...
- (void)refresh:(NSDictionary *)addressDict {
    ...
    serverURL = [[NSString alloc]initWithFormat:@"%@/deals/search?type=A&city=%@&zipcode=%@&coordinate=%@&searchingradius=%@&pageIndex=0&noOfdeals=0&restName=&address=&version=%i",SERVER_URL,[arr objectAtIndex:0],[arr objectAtIndex:1],[arr objectAtIndex:2],[sharedDelegate.searchingradies objectForKey:@"radies"]];
...
-(void)showMoreAction:(id)sender {
    ...
    @try {
        if ([allEntries count]!=noOfOffers) {
            ...
            serverURL = [[NSString alloc]initWithFormat:@"%@/deals/search?type=A&city=%@&zipcode=%@&coordinate=%@&searchingradius=%@&pageIndex=%i&noOfdeals=0&restName=&address=",SERVER_URL,[addressArray objectAtIndex:0],[addressArray objectAtIndex:1],[addressArray objectAtIndex:2],[sharedDelegate.searchingradies objectForKey:@"radies"],pageIndex];
